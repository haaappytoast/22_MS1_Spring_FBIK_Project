...
public class jacobArmOptim : MonoBehaviour
{
    ...
    float loss_func(ref double[] x)
    {
        ...
        localRotation[1] = Quaternion.AngleAxis((float)x[3] * Mathf.Rad2Deg, Vector3.up);
    ...
    double[] get_dL_dx(int trf_idx, double[] x, Vector3 dL_dp)
    {
        ...
        for (int rot_idx = 0; rot_idx < 3; ++rot_idx)
        {
            ...
            Vector3 dp_dx = get_dp_dx(0, b, xh, expInv);
    ...
    double get_dL_dx(int trf_idx, double x, Vector3 dL_dp)
    {
        ...
        Vector3 aaxis = new Vector3(0, (float) x, 0) * Mathf.Rad2Deg;
    ...
    public void Solve_limb_IK_RotVec_grad()
    {
        ...
        // hinge joint
        SetPose(new double[] { 0, x[3], 0 }, Ts[1]);
