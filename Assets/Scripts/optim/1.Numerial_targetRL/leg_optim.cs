...
public class leg_optim : MonoBehaviour
{
    ...
    float[] FminLimit = new float[] { 0.0f, 0.0f };
    float[] FmaxLimit = new float[] { 125f, 125f };

    void Start()
    {
        //if (side == Side.left)
        //{
        //    FminLimit = new float[] { -125f, -125f };
        //    FmaxLimit = new float[] { 0.0f, 0.0f };
        //}
    ...
    {
        var tar = (obj as leg_optim).target;
        ...
        localRotation[1] = Quaternion.Euler((float)x[3], 0 , 0);
        ...
        if (contraints)
        {
            for (int i = 1; i < 2; i++)
            {
                float jntAngle = (float)dRotation[i].eulerAngles.x;
                if (jntAngle > 180)
                {
    ...
    public void Solve_limb_IK_Euler(bool useConstraints)
    {
        ...
        // hinge joint
        SetPose(x[3], Vector3.right, Ts[1]);
    ...
    void IK_Loss_Function_RotVec(double[] x, ref double loss, object obj)
    {
        ...
        localRotation[1] = Quaternion.AngleAxis((float)x[3], Vector3.right);
    ...
    public void Solve_limb_IK_RotVec()
    {
        ...
        // hinge joint
        SetPose(new double[] { x[3], 0, 0 }, Ts[1], "RotVec");
